...
from Database import WizardsUnite, WizardsUniteEventType, WizardsUniteEvent

LOGGER = logging.getLogger(__name__)


def generate_embed(event: WizardsUniteEvent, author: Member) -> Embed:
    embed = Embed(title=event.name, colour=load_colour(event.event_type.colour_code))

    embed.add_field(name='Game', value='Harry Potter: Wizards Unite')
...
class WizardsUniteCog(commands.Cog, name='Harry Potter: Wizards Unite Commands'):
    ...
    async def wizards_group(self, ctx, username: str):
        with db_session:
            result = WizardsUnite.get(username=username)
    ...
    async def join(self, ctx, username: str, email: str):
        with db_session:
            WizardsUnite.safe_insert(username, email)
            await ctx.message.add_reaction('✅')
    ...
    async def edit_types(self, ctx, username: str, *type_names: str):
        with db_session:
            user = WizardsUnite.get(username=username)
            if user:
                for type_name in type_names:
                    event_type = WizardsUniteEventType.find(name=type_name)
                    if event_type:
    ...
    async def event_group(self, ctx):
        with db_session:
            results = WizardsUniteEvent.select()[:]
            if results:
    ...
    async def event_type_list(self, ctx):
        with db_session:
            await ctx.send(', '.join([x.name for x in WizardsUniteEventType.select()[:]]))
            await ctx.message.delete()
    ...
    async def create_event(self, ctx, name: str, start_time: str, end_time: str, type_name: str,
        ...
        with db_session:
            event = WizardsUniteEvent.get(name=name)
            if event:
                LOGGER.warning(f"{name} already exists")
                await ctx.message.add_reaction('❎')
            else:
                event_type = WizardsUniteEventType.find(type_name)
                if event_type:
                    try:
                        ...
                        if not local_timezone:
                            ...
                            end_datetime = datetime.strptime(
                                timezone('America/Los_Angeles').localize(end_datetime).astimezone(
                                    timezone('Pacific/Auckland')).strftime('%y-%m-%d %H:%M'), '%y-%m-%d %H:%M')
                        event = WizardsUniteEvent.safe_insert(name, start_datetime, end_datetime, event_type,
                                                              all_day=all_day)
...
def create_google_event(event: WizardsUniteEvent):
    event_json = {
        'summary': event.name,
        'start': {
        ...
        'attendees': [x.event_dict() for x in WizardsUnite.select(lambda x: event.event_type in x.event_types)],
        'reminders': {
            'useDefault': False,
            'overrides': []
        },
    ...
    insert_calendar_event('Wizards Unite', event_json)
