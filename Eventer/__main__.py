...
def get_arguments() -> Namespace:
    ...
    parser.add_argument('-t', '--test', action='store_true', help='Stops Events from being created in Google Calendar')
    parser.add_argument('-c', '--catan', action='store_true', help='Reads and Creates the events for the Catan: World Explorers Game')
    parser.add_argument('-p', '--pokemon', action='store_true', help='Reads and Creates the events for the Pokemon Go Game')
    parser.add_argument('-w', '--wizards', action='store_true', help='Reads and Creates the events for the Harry Potter: Wizards Unite Game')
...
def main():
    ...
    for game_title, file_events in all_events.items():
        ...
        for file_event in file_events:
            ...
            filtered = filter(lambda x: x['summary'] == file_event.name and x['start']['dateTime'] == file_event.start_time_localized(), calendar_events)
            exists = next(filtered, None)
            if exists:
                update_event(service=service, file_event=file_event, calendar_event=exists, attendees=filtered_attendees, calendar_id=CONFIG[game_title]['Google Calendar ID'])
            else:
                create_event(service, file_event, attendees=filtered_attendees, calendar_id=CONFIG[game_title]['Google Calendar ID'])
